    ...
    const verifyAndProcess = async () => {
      try {
        ...
        if (data.success) {
          ...
          // We fetch the order items to remove them specifically from Firestore
          ...
          setError("Reconciliation taking longer than expected. We'll confirm via email.");
          setIsVerifying(false);
        }
      } catch (err) {
        setError("Network error. Your acquisition is still being processed.");
        setIsVerifying(false);
      }
    };

    verifyAndProcess();
  }, [orderId, user, firestore]);
          ...
          <AnimatePresence mode="wait">
            {isVerifying ? (
              ...
              >
                ...
                <h1 className="text-3xl font-black text-primary uppercase tracking-tight">Securing Acquisition</h1>
                <p className="text-muted-foreground font-medium">Reconciling your payment with the gateway...</p>
              </motion.div>
            ) : (
              <motion.div 
                key="success"
              ...
              >
                ...
                <div className="space-y-3">
                  <h1 className="text-4xl font-black text-primary tracking-tight">Payment Successful</h1>
                  <p className="text-muted-foreground font-medium">Your artisan pieces are officially confirmed.</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-8 border-y border-dashed border-primary/10">
                  <div className="flex items-center gap-4 text-left">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold">Encrypted & Confirmed</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 text-left">
                    <div className="h-10 w-10 rounded-xl bg-primary/5 flex items-center justify-center text-primary shrink-0">
                    ...
                    <div>
                      <p className="text-[10px] font-black uppercase text-muted-foreground">Inventory</p>
                      <p className="text-sm font-bold">Stock Reserved</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6 pt-4">
                  <div className="text-xs text-muted-foreground font-bold">
                    Redirecting to your order history in <span className="text-primary text-lg tabular-nums">{countdown}s</span>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    ...
                    >
                      View Order Now <ArrowRight className="ml-2 h-4 w-4" />
                    </Button>
                    <Button 
                      variant="ghost"
