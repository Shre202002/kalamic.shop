...
export default function ProductsManagement() {
  ...
  const columns: GridColDef[] = useMemo(() => [
    ...
    { field: 'price', headerName: 'Price', width: 110, renderCell: (params) => `₹${(params.value ?? 0).toLocaleString()}` },
    ...
    { field: 'is_active', headerName: 'Live', width: 80, renderCell: (params) => <Switch checked={!!params.value} size="small" onChange={() => toggleProductVisibility(user!.uid, params.row._id, !params.value).then(load)} /> },
    { field: 'is_featured', headerName: 'Star', width: 80, renderCell: (params) => <Switch checked={!!params.value} size="small" color="secondary" onChange={() => toggleProductFeatured(user!.uid, params.row._id, !params.value).then(load)} /> },
    {
      field: 'actions',
      headerName: '',
      width: 100,
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', mb: 5 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', bgcolor: 'white', px: 2, borderRadius: 3, border: '1px solid rgba(0,0,0,0.05)' }}>
            ...
            <InputBase placeholder="Search pieces..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </Box>
          <Button variant="contained" startIcon={<Add />} onClick={() => handleOpenDialog()} sx={{ borderRadius: 3, px: 3, fontWeight: 800 }}>New Piece</Button>
        </Box>
      </Box>
                ...
                <Box>
                  {(editingProduct.images || []).map((img: any, idx: number) => (
                    <Paper key={idx} variant="outlined" sx={{ p: 2, mb: 2, display: 'flex', gap: 2, alignItems: 'center' }}>
                      <TextField fullWidth label="URL" value={img.url} onChange={(e) => {
                      ...
                      <IconButton color="error" onClick={() => setEditingProduct({...editingProduct, images: editingProduct.images.filter((_: any, ii: number) => ii !== idx)})}><Delete /></IconButton>
                    </Paper>
                  ))}
                ...
                <Box>
                  {(editingProduct.specifications || []).map((spec: any, idx: number) => (
                    <Box key={idx} sx={{ display: 'flex', gap: 2, mb: 2 }}>
                      <TextField label="Key" value={spec.key} onChange={(e) => {
                      ...
                      <IconButton color="error" onClick={() => setEditingProduct({...editingProduct, specifications: editingProduct.specifications.filter((_: any, ii: number) => ii !== idx)})}><Delete /></IconButton>
                    </Box>
                  ))}
